...
type
  ...
  public
    constructor Create(const Key, Salt: TBytes; Cost: Integer);

    procedure ExpandKey(const Key, Salt: TBytes);
    procedure EncryptBlock(var L, R: Cardinal);
  end;
...
const
  ...
  S_BOX: array[0..3, 0..255] of Cardinal = (
        {SBox0}
				(
...
constructor TBlowfish.Create(const Key, Salt: TBytes; Cost: Integer);
var
  Rounds, I: Integer;
begin
  InitP;
...
function TBlowfish.F(X: Cardinal): Cardinal;
begin
  Result :=
    ((S[0, X shr 24] + S[1, (X shr 16) and $FF]) xor
      S[2, (X shr 8) and $FF]) +
      S[3, X and $FF];
end;

procedure TBlowfish.ExpandKey(const Key, Salt: TBytes);
var
  I: Integer;
  L, R: Cardinal;
begin
  ...
  for I := 0 to 17 do
    P[I] := P[I] xor Key[I mod Length(Key)];

  L := 0; R := 0;

  for I := 0 to 17 div 2 do
  begin
    EncryptBlock(L, R);
    P[2*I] := L;
    P[2*I+1] := R;
...
procedure TBlowfish.EncryptBlock(var L, R: Cardinal);
var
  I: Integer;
begin
  for I := 0 to 15 do
